...
func (eh *EventHandler) UploadPhotos(ctx echo.Context) error {
	eventUID := ctx.Param("id")
	if eventUID == "" {
		err := errors.New("event id is empty")
		eh.logger.WithError(err).Errorf("failed to upload photos")
		return ctx.JSON(http.StatusBadRequest, err.Error())
	}
	...
	err = eh.useCase.UploadPhotos(ctx.Request().Context(), mf.File, eventUID)
	if err != nil {
		eh.logger.WithError(err).Errorf("failed to upload photos")
		return ctx.JSON(http.StatusInternalServerError, err.Error())
	}

	return ctx.JSON(http.StatusOK, "successfully uploaded photos")
}
...
func (eh *EventHandler) DeletePhotos(ctx echo.Context) error {
	eventUID := ctx.Param("id")
	if eventUID == "" {
		err := errors.New("event id is empty")
		eh.logger.WithError(err).Errorf("failed to upload photos")
	...
	var photoLinks photoData
	if err := ctx.Bind(&photoLinks); err != nil {
		eh.logger.WithError(err).Errorf("failed to bind photo data")
		return ctx.JSON(http.StatusBadRequest, err.Error())
	}

	err := eh.useCase.DeletePhotos(ctx.Request().Context(), photoLinks.Links, eventUID)
	if err != nil {
		eh.logger.WithError(err).Errorf("failed to delete photos")
		return ctx.JSON(http.StatusInternalServerError, err.Error())
	}

	return ctx.JSON(http.StatusOK, "successfully deleted photos")
}
